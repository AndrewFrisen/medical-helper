...
/medcard #playerID #healID #timeID 1000]]  
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("recepts", funCMD.rec)
			...
			if id:find("(%d+)") then
				thread = lua_thread.create(function()
					...
					if vaccination == 1 then
						...
						wait(tonumber(player_info.msg_delay))
						sampSendChat("Стоимость полной вакцинации составляет 600.000, если вы согласны то передайте мне вашу мед. карту")
						wait(tonumber(player_info.msg_delay))
